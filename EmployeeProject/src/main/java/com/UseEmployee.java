...
public class UseEmployee{
	...
	public static void main(String[] args) {
		...
		while(menuSelection != 6) {
			//menuDisplay();
			menuSelection=optionSelected(sca);
	...
	public static void choice2(EmployeeServiceImpl solution,Scanner sca) {
		try {
			...
			Employee su= solution.list.stream()
					...
					.findFirst()
					.orElse(null);
			System.out.println("Employee Salary for "+solution.findByEmployeeNo(EmployeeNum2).getEmployeeName()+"for the year is:"+solution.calculateYearlySalary(su));
		}catch(Exception e) {
	...
	public static void choice5(EmployeeServiceImpl solution,Scanner sca) {
		...
		Employee suti = solution.list.stream()
				...
				.findFirst()
				.orElse(null);
		//Employee suti=solution.list.get(EmployeeNum1);
		if(suti ==null) {
			System.out.println("Error arraylist index is out of bounds");
	...
	public static int optionSelected(Scanner sca) {
		menuDisplay();
		// @SuppressWarnings("resource")
		//Scanner scanObj = new Scanner(System.in);
		System.out.println("Insert option: ");
		int menuSelection = sca.nextInt();
